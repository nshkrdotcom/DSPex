...
defmodule Dspy.BootstrapFewShot do
  @moduledoc """
  ...
  - `max_rounds` - Maximum number of bootstrap attempts per training example. Each round after the first uses a fresh rollout with `temperature=1.0` to bypass caches and gather diverse traces. If a successful bootstrap is found on any round, the example is accepted and the optimizer moves to the next one. Defaults to 1. (type: `integer()`)
