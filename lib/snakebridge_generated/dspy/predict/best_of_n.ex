...
defmodule Dspy.Predict.BestOfN do
  ...
  - `reward_fn` - A callable that takes the input kwargs dict and a `Prediction`, and returns a scalar float reward score.
  - `threshold` - If an attempt's reward is at or above this value, that prediction is returned immediately without further attempts.
  - `fail_count` - Number of allowed failures before raising an exception. Defaults to `N` if not provided.

  ## Examples

      iex> import dspy
      iex> dspy.configure(lm=dspy.LM("openai/gpt-4o-mini"))
      iex> qa = dspy.ChainOfThought("question -> answer")
      iex> def one_word_answer(args, pred):
      ...> return 1.0 if len(pred.answer.split()) == 1 else 0.0
      iex> best_of_3 = dspy.BestOfN(module=qa, N=3, reward_fn=one_word_answer, threshold=1.0)
      iex> result = best_of_3(question="What is the capital of Belgium?")
      iex> print(result.answer)  # Brussels
  """
  ...
  - `reward_fn` (term())
  - `threshold` (float())
  - `fail_count` (integer() | nil default: None)
  """
  @spec new(term(), integer(), term(), float(), list(term()), keyword()) ::
  ...
  @doc """
  Python method `BestOfN._base_init`.
  ...
  @doc """
  Python method `BestOfN._set_lm_usage`.
  ...
  @doc """
  Python method `BestOfN.dump_state`.
  ...
  @doc """
  Python method `BestOfN.load_state`.
  ...
  @doc """
  Python method `BestOfN.parameters`.
