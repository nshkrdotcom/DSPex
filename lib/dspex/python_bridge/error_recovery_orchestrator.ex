defmodule DSPex.PythonBridge.ErrorRecoveryOrchestrator do
  ...
  def init(opts) do
    ...
    Logger.info("ErrorRecoveryOrchestrator started with #{map_size(state.recovery_strategies)} strategies")
    {:ok, state}
  end
  ...
  def handle_call({:handle_error, error, context}, from, state) do
    if map_size(state.active_recoveries) >= state.config.max_concurrent_recoveries do
      ...
      case strategy.type do
        :abandon ->
          Logger.info("Error recovery abandoned based on strategy: #{inspect(wrapped_error.error_category)}")
        ...
        _ ->
          ...
          Logger.info("Started recovery #{recovery_id} for #{wrapped_error.error_category} with strategy #{strategy.type}")
    ...
    case Map.get(state.active_recoveries, recovery_id) do
      ...
      recovery ->
        ...
        if recovery.task_ref do
          Task.shutdown(recovery.task_ref, :brutal_kill)
        end
  ...
  @spec load_recovery_strategies(keyword()) :: map()
  ...
  defp execute_failover_recovery(error, strategy) do
    case strategy.fallback_adapter do
      nil ->
        {:error, :no_fallback_adapter}
  ...
  @spec handle_recovery_completion(String.t(), recovery_state(), term(), %__MODULE__{}) :: {:noreply, %__MODULE__{}}
  defp handle_recovery_completion(recovery_id, recovery, result, state) do
    ...
    # Update metrics
    {_metric_key, new_metrics} = case result do
      {:ok, _} ->
