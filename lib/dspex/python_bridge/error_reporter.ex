defmodule DSPex.PythonBridge.ErrorReporter do
  ...
  @default_config %{
    ...
    circuit_open_threshold: 3,        # Alert after 3 circuits open
    alert_window_ms: 60_000,          # 1 minute sliding window
    min_events_for_alert: 10,         # Minimum events before calculating rates
    max_window_events: 1000,          # Maximum events to keep in memory
    cleanup_interval: 300_000,        # 5 minutes
    monitoring_enabled: true,
    alert_destinations: [:logger],
    alert_cooldown_ms: 300_000        # 5 minute cooldown between same alerts
  }
  ...
  def init(opts) do
    ...
    if config.monitoring_enabled do
      ...
      Logger.info("ErrorReporter monitoring enabled with #{length(config.alert_destinations)} alert destinations")
    else
  ...
  defp handle_circuit_opened_event(metadata, timestamp, state) do
    ...
    alert = %{
      type: :circuit_opened,
      message: "Circuit breaker '#{circuit_name}' opened after #{circuit_info.failure_count} failures",
  ...
  defp handle_retry_exhausted_event(measurements, metadata, _timestamp, state) do
    ...
    Logger.error("Retry attempts exhausted: #{final_attempt}/#{max_attempts} for #{inspect(metadata[:error])}")
  ...
  defp check_and_send_alerts(state) do
    ...
    recent_events = :queue.to_list(state.event_window)
    ...
    if total_events >= state.config.min_events_for_alert do
      ...
      if error_rate >= state.config.error_rate_threshold do
        alert = %{
          ...
          message: "High error rate detected: #{Float.round(error_rate * 100, 1)}% (#{error_events}/#{total_events} in last #{div(state.config.alert_window_ms, 1000)}s)",
    ...
    open_circuits = state.circuit_states
                   |> Map.values()
  ...
  defp should_send_alert(alert, state) do
    ...
    recent_alerts = :queue.to_list(state.alert_history)
  ...
  defp cleanup_old_events(state) do
    now = System.monotonic_time(:millisecond)
    cutoff = now - (state.config.alert_window_ms * 2)  # Keep 2x window for analysis
    
    # Clean event window
    new_window = :queue.to_list(state.event_window)
                |> Enum.filter(&(&1.timestamp >= cutoff))
                |> :queue.from_list()
    
    # Clean alert history (keep last 100)
    new_alert_history = :queue.to_list(state.alert_history)
                       |> Enum.take(-100)
                       |> :queue.from_list()
