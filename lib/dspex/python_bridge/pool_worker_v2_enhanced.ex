defmodule DSPex.PythonBridge.PoolWorkerV2Enhanced do
  ...
  def handle_checkout(checkout_type, from, worker, pool_state) do
    case WorkerStateMachine.can_accept_work?(worker.state_machine) do
      ...
      false ->
        Logger.debug("Worker #{worker.worker_id} not ready for checkout, state: #{worker.state_machine.state}, health: #{worker.state_machine.health}")
  ...
  def terminate_worker(reason, worker, pool_state) do
    ...
    {:ok, final_state_machine} = WorkerStateMachine.transition(
      worker.state_machine,
      :terminated,
      :terminate,
      %{reason: reason}
    ...
    Logger.debug("Worker #{worker.worker_id} terminated, final state: #{final_state_machine.state}")
  ...
  defp perform_checkout(checkout_type, from, worker, pool_state) do
    Logger.error("Worker #{worker.worker_id} invalid checkout type: #{inspect(checkout_type)} from #{inspect(from)}")
  ...
  defp handle_error_checkin(worker, reason, pool_state) do
    ...
    Logger.warning("Worker #{worker.worker_id} error checkin: #{inspect(reason)}, failures: #{failures}")
    
    if failures >= @max_health_failures do
      # Too many failures, remove worker
      Logger.error("Worker #{worker.worker_id} exceeded max failures (#{@max_health_failures}), removing")
  ...
  defp perform_initialization(worker) do
    ...
    try do
      unless Port.command(worker.port, request) do
  ...
  defp wait_for_init_response(worker, request_id) do
    receive do
          ...
          {:ok, ^request_id, response} ->
            case response do
              ...
              response_map when is_map(response_map) ->
                Logger.debug("Enhanced worker #{worker.worker_id} init successful with response: #{inspect(response_map)}")
                {:ok, worker}
      ...
      {port, {:exit_status, status}} when port == worker.port ->
        Logger.error("Enhanced worker #{worker.worker_id} port exited during init with status #{status}")
  ...
  defp execute_health_check(worker) do
    ...
    try do
      unless Port.command(worker.port, request) do
        raise "Port.command/2 failed during health check"
      end
  ...
  defp handle_health_check_failure(worker, reason) do
    failures = worker.health_check_failures + 1
    Logger.warning("Worker #{worker.worker_id} health check failed: #{inspect(reason)}, failures: #{failures}")
    
    new_state_machine = 
      if failures >= @max_health_failures do
        case WorkerStateMachine.transition(worker.state_machine, :terminating, :health_check_failed) do
          {:ok, sm} -> 
            Logger.error("Worker #{worker.worker_id} marked for termination due to health failures")
            sm
          _ -> 
            worker.state_machine
        end
