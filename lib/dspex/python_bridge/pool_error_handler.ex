defmodule DSPex.PythonBridge.PoolErrorHandler do
  ...
  defp categorize_error(error) do
    case error do
      ...
      {:resource_error, _} -> :resource_error  # Direct resource error tuples
      
      # Connection errors
      {:connection_error, _} -> :connection_error  # Direct connection error tuples
      
      # Communication errors  
      {:communication_error, _} -> :communication_error  # Direct communication error tuples
      
      # Python errors
      {:python_error, _} -> :python_error  # Direct python error tuples
      
      # Generic error patterns
      {:error, inner} -> categorize_error(inner)
      
      # Common retryable errors
      {:simple_error, _} -> :communication_error  # Make simple errors retryable
  ...
  @spec determine_recovery_strategy(error_category(), error_severity(), map()) :: recovery_strategy()
  ...
  @spec determine_strategy_by_category(error_category(), error_severity(), map()) :: recovery_strategy()
