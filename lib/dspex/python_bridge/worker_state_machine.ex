defmodule DSPex.PythonBridge.WorkerStateMachine do
  ...
  @type transition_reason :: 
    :init_complete | :checkout | :checkin_success | :checkin_error | 
    :health_check_failed | :health_check_passed | :health_restored |
    :shutdown | :terminate | :error | :timeout | :recovery_degrade
  ...
  defp do_transition(machine, new_state, reason, metadata) do
    ...
    try do
      ...
      WorkerMetrics.record_transition(machine.worker_id, machine.state, new_state, duration, metadata)
    rescue
      _ -> :ok  # Don't fail transitions due to metrics issues
    end
