defmodule DSPex.PythonBridge.CircuitBreaker do
  ...
  @default_config %{
    ...
    timeout: 60_000,          # Time before half-open attempt (60 seconds)
    ...
    case circuit.state do
      ...
      :open ->
        ...
        if should_attempt_reset?(circuit) do
          ...
          Logger.debug("Circuit #{circuit_name} is open, rejecting request (#{time_remaining}ms until retry)")
          
          wrapped_error = PoolErrorHandler.wrap_pool_error(error, %{
      ...
      :half_open ->
        ...
        if circuit.half_open_count < circuit.config.half_open_requests do
          Logger.debug("Circuit #{circuit_name} allowing half-open request (#{circuit.half_open_count + 1}/#{circuit.config.half_open_requests})")
    ...
    case circuit.state do
      ...
      :half_open ->
        ...
        if new_count >= circuit.config.success_threshold do
          ...
          Logger.info("Circuit #{circuit.name} closed after successful recovery (#{new_count} successes)")
          ...
          Logger.debug("Circuit #{circuit.name} half-open success #{new_count}/#{circuit.config.success_threshold}")
          %{circuit | success_count: new_count}
        end
        
      :open ->
        # Shouldn't happen, but handle gracefully
        Logger.warning("Circuit #{circuit.name} received success while open - this should not happen")
        circuit
    end
  end
  ...
  defp handle_failure(circuit, error) do
    ...
    Logger.debug("Circuit #{circuit.name} failure #{new_failure_count} recorded: #{inspect(error)}")
    ...
    case circuit.state do
      :closed when new_failure_count >= circuit.config.failure_threshold ->
        # Open the circuit
        Logger.error("Circuit #{circuit.name} opened after #{new_failure_count} failures (threshold: #{circuit.config.failure_threshold})")
