defmodule DSPex.PythonBridge.WorkerRecovery do
  ...
  def determine_strategy(failure_reason, worker_state, context \\ %{}) do
    ...
    Logger.debug(
      "Determining recovery strategy for worker #{worker_state.worker_id}, failure: #{inspect(failure_reason)}"
    )
    ...
    cond do
      ...
      match?({:port_exited, _}, failure_reason) ->
        ...
        %{
          action: :remove,
          delay: 0,
          metadata: %{reason: :port_failure, exit_status: elem(failure_reason, 1)}
        }
      ...
      match?({:checkout_failed, _}, failure_reason) ->
        ...
        %{
          action: :remove,
          delay: 0,
          metadata: %{reason: :checkout_failure, details: elem(failure_reason, 1)}
        }
      ...
      match?({:max_failures_exceeded, _}, failure_reason) ->
        ...
        %{
          action: :remove,
          delay: 0,
          metadata: %{reason: :max_failures, details: elem(failure_reason, 1)}
        }
      ...
      match?({:worker_not_ready, _}, failure_reason) ->
        ...
        %{
          action: :remove,
          delay: 0,
          metadata: %{reason: :not_ready, state: elem(failure_reason, 1)}
        }
      ...
      ErrorHandler.should_retry?(wrapped_error) ->
        ...
        Logger.info(
          "Worker #{worker_state.worker_id} error is recoverable, retrying after #{retry_delay}ms"
        )
      ...
      true ->
        ...
        %{
          action: :remove,
          delay: 0,
          metadata: %{reason: :non_recoverable, error_type: wrapped_error.type}
        }
    end
  end
  ...
  def execute_recovery(strategy, worker_state, pool_state) do
    case strategy.action do
      :retry ->
        Logger.info(
          "Retrying worker #{worker_state.worker_id} operation after #{strategy.delay}ms"
        )
  ...
  defp determine_health_failure_strategy(worker_state, _failure_reason, wrapped_error) do
    ...
    # This should match the constant in enhanced worker
    max_failures = 3

    if failures >= max_failures do
      Logger.error(
        "Worker #{worker_state.worker_id} health check failures exceeded limit (#{failures}/#{max_failures})"
      )

      %{
        action: :remove,
        delay: 0,
        metadata: %{reason: :health_check_limit, failure_count: failures}
      }
    else
      Logger.warning(
        "Worker #{worker_state.worker_id} health check failed (#{failures}/#{max_failures}), degrading"
      )
      ...
      %{
        action: :degrade,
        delay: delay,
        metadata: %{reason: :health_degraded, failure_count: failures}
      }
    end
  end
  ...
  defp determine_timeout_strategy(worker_state, failure_reason, wrapped_error) do
    if ErrorHandler.should_retry?(wrapped_error) do
      ...
      Logger.info(
        "Worker #{worker_state.worker_id} timeout is recoverable, retrying after #{delay}ms"
      )
  ...
  defp degrade_worker(worker_state, pool_state, metadata) do
    ...
    case WorkerStateMachine.transition(
      ...
      {:error, reason} ->
        Logger.error(
          "Failed to transition worker #{worker_state.worker_id} to degraded state: #{inspect(reason)}"
        )
