defmodule DSPex.PythonBridge.RetryLogic do
  ...
  @type backoff_strategy :: :linear | :exponential | :fibonacci | :decorrelated_jitter | function()
  ...
  """
  @spec calculate_delay(pos_integer(), backoff_strategy(), pos_integer(), pos_integer(), boolean()) :: non_neg_integer()
  def calculate_delay(attempt, strategy, base_delay, max_delay, jitter \\ true) do
    ...
    final_delay = if jitter and strategy != :decorrelated_jitter do
  ...
  @spec do_retry(function(), pos_integer(), pos_integer(), backoff_strategy(), pos_integer(), pos_integer(), atom() | nil, boolean(), map(), term()) :: {:ok, term()} | {:error, term()}
  defp do_retry(fun, attempt, max_attempts, backoff, base_delay, max_delay, circuit, jitter, context, _last_error) do
    # Execute through circuit breaker if configured and available
    result = if circuit && circuit_breaker_available?() do
      ...
      try do
        case fun.() do
          ...
          {:error, _} = error -> error
          other -> {:ok, other}  # Treat non-tuple returns as success
        end
      catch
    ...
    case result do
      {:ok, value} ->
        if attempt > 1 do
          ...
          emit_telemetry(:retry_success, %{attempt: attempt, total_attempts: max_attempts}, context)
        end
        {:ok, value}
      ...
      {:error, error} ->
        ...
        if attempt < max_attempts and should_retry?(wrapped_error, attempt) do
          ...
          Logger.warning("Retry attempt #{attempt}/#{max_attempts} failed, retrying in #{delay}ms: #{inspect(error)}")
          emit_telemetry(:retry_attempt, %{attempt: attempt, delay: delay}, Map.merge(context, %{error: error}))
          
          Process.sleep(delay)
          do_retry(fun, attempt + 1, max_attempts, backoff, base_delay, max_delay, circuit, jitter, context, wrapped_error)
        else
          Logger.error("All retry attempts exhausted (#{attempt}/#{max_attempts}): #{inspect(error)}")
          emit_telemetry(:retry_exhausted, %{final_attempt: attempt, max_attempts: max_attempts}, Map.merge(context, %{error: error}))
          {:error, wrapped_error}
        end
    end
  end
  ...
  @spec calculate_decorrelated_jitter(pos_integer(), pos_integer(), pos_integer()) :: non_neg_integer()
  defp calculate_decorrelated_jitter(_attempt, base_delay, max_delay) do
    # AWS-style decorrelated jitter
    # See: https://aws.amazon.com/blogs/architecture/exponential-backoff-and-jitter/
    
    last_delay = case Process.get(:last_retry_delay) do
      nil -> base_delay
      delay -> delay
    end
  ...
  defp add_jitter(delay) do
    ...
    jitter_range = div(delay, 4)  # 25% of delay
