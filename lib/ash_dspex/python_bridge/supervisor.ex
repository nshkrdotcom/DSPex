defmodule AshDSPex.PythonBridge.Supervisor do
  @moduledoc """
  ...
  ```
  AshDSPex.PythonBridge.Supervisor
  ├── AshDSPex.PythonBridge.Bridge (permanent)
  └── AshDSPex.PythonBridge.Monitor (permanent)
  ```

  ## Usage

      # Start the bridge supervisor
      {:ok, pid} = AshDSPex.PythonBridge.Supervisor.start_link()

      # Get supervisor status
      children = AshDSPex.PythonBridge.Supervisor.which_children()

      # Stop the supervisor
      :ok = AshDSPex.PythonBridge.Supervisor.stop()
  ...
  ## Examples
      ...
      {:ok, pid} = AshDSPex.PythonBridge.Supervisor.start_link()
      {:ok, pid} = AshDSPex.PythonBridge.Supervisor.start_link(name: MyBridgeSupervisor)
  """
  ...
  ## Examples
      ...
      children = AshDSPex.PythonBridge.Supervisor.which_children()
      # => [
      #   {AshDSPex.PythonBridge.Bridge, #PID<0.123.0>, :worker, [AshDSPex.PythonBridge.Bridge]},
      #   {AshDSPex.PythonBridge.Monitor, #PID<0.124.0>, :worker, [AshDSPex.PythonBridge.Monitor]}
      # ]
  """
  ...
  ## Examples
      ...
      count = AshDSPex.PythonBridge.Supervisor.count_children()
  ...
  ## Examples
      ...
      :ok = AshDSPex.PythonBridge.Supervisor.terminate_child(AshDSPex.PythonBridge.Bridge)
  """
  ...
  ## Examples
      ...
      {:ok, pid} = AshDSPex.PythonBridge.Supervisor.restart_child(AshDSPex.PythonBridge.Bridge)
  """
  ...
  ## Examples
      ...
      :ok = AshDSPex.PythonBridge.Supervisor.stop(pid)
  """
  @spec stop(pid()) :: :ok
  ...
  ## Examples
      ...
      :ok = AshDSPex.PythonBridge.Supervisor.stop(pid, :shutdown, 1000)
  """
  ...
  ## Examples
      ...
      status = AshDSPex.PythonBridge.Supervisor.get_system_status()
      # => %{
      #   supervisor: :running,
  ...
  def get_system_status do
    ...
    bridge_status = get_child_status(AshDSPex.PythonBridge.Bridge)
    monitor_status = get_child_status(AshDSPex.PythonBridge.Monitor)
  ...
  def init(opts) do
    ...
    monitor_name = Keyword.get(opts, :monitor_name, AshDSPex.PythonBridge.Monitor)

    Logger.info("Starting Python bridge supervisor with config: #{inspect(config)}")

    children = [
      # Start bridge first
      {AshDSPex.PythonBridge.Bridge, [restart: config.bridge_restart, name: bridge_name]},
  ...
  defp build_config(opts) do
    app_config = Application.get_env(:ash_dspex, :python_bridge_supervisor, %{})
    ...
    try do
      case child_module do
        AshDSPex.PythonBridge.Bridge ->
          AshDSPex.PythonBridge.Bridge.get_status()
  ...
  ## Examples
      ...
      :ok = AshDSPex.PythonBridge.Supervisor.graceful_shutdown()
      :ok = AshDSPex.PythonBridge.Supervisor.graceful_shutdown(timeout: 10_000)
  """
  ...
  def graceful_shutdown(opts \\ []) do
    ...
    case terminate_child(AshDSPex.PythonBridge.Monitor) do
    ...
    # Terminate bridge
    case terminate_child(AshDSPex.PythonBridge.Bridge) do
  ...
  ## Examples
      ...
      case AshDSPex.PythonBridge.Supervisor.system_health_check() do
  ...
  defp check_bridge_health(issues) do
    case get_child_status(AshDSPex.PythonBridge.Bridge) do
  ...
  defp check_monitor_health(issues) do
    case get_child_status(AshDSPex.PythonBridge.Monitor) do
