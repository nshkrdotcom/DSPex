defmodule AshDSPex.Adapters.PythonPort do
  @moduledoc """
  Python port adapter for Layer 3 testing.

  This adapter wraps the existing `AshDSPex.PythonBridge.Bridge` to provide
  ...
  ## Usage
      ...
      adapter = AshDSPex.Adapters.Registry.get_adapter(:python_port)
      {:ok, program_id} = adapter.create_program(%{signature: %{...}})

      # Direct usage
      {:ok, result} = AshDSPex.Adapters.PythonPort.execute_program("program_id", %{input: "test"})
  """
