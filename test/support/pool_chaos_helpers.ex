defmodule DSPex.PoolChaosHelpers do
  ...
  """
  @spec inject_worker_failure(map(), atom() | float() | list(), keyword()) ::
          {:ok, map()} | {:error, term()}
  ...
  def verify_pool_recovery(pool_info, pre_chaos_state, timeout \\ 30_000) do
    ...
    case wait_for_pool_ready(pool_info.pool_name, pool_info.pool_name, timeout) do
      {:ok, :ready} ->
        ...
        case wait_for_workers_initialized(
               pool_info.pool_name,
               pre_chaos_state.expected_workers,
               timeout
             ) do
          {:ok, worker_state} ->
  ...
  def chaos_test_orchestrator(pool_info, chaos_scenarios, opts \\ []) do
    ...
    Logger.info(
      "Starting chaos orchestration: #{length(chaos_scenarios)} scenarios, mode: #{execution_mode}"
    )
  ...
  defp attempt_worker_failure(target, pool_info) do
    ...
    try do
        ...
        for _i <- 1..5 do
          Task.async(fn ->
            SessionPoolV2.execute_anonymous(
              # This operation should cause worker issues
              :crash_worker,
              %{chaos_test: true, target: target.index},
              pool_name: pool_info.actual_pool_name,
              # Short timeout to cause failures
              timeout: 1000
            )
          end)
        end
      ...
      running_count =
        Enum.count(failure_operations, fn task ->
  ...
  defp execute_chaos_scenarios_sequential(pool_info, scenarios, verify_recovery_between) do
    {results, _final_state} =
      Enum.reduce(scenarios, {[], capture_pool_state(pool_info)}, fn scenario,
                                                                     {acc, current_state} ->
