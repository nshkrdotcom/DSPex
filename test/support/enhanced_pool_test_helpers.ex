defmodule DSPex.EnhancedPoolTestHelpers do
  ...
  def setup_isolated_pool(pool_name, opts \\ []) do
    ...
    case start_test_pool(pool_opts) do
      %{pool_pid: _pool_pid} = pool_info ->
        ...
        performance_context = if performance_monitoring do
        ...
        session_context = if session_affinity do
          setup_session_affinity_testing(pool_info)
        else
          %{}
        end
        
        enhanced_info = Map.merge(pool_info, %{
  ...
  def warm_pool_workers(pool_name, worker_count, opts \\ []) do
    ...
    case pre_warm_pool(pool_name, worker_count) do
      :ok ->
        warmup_time = if track_performance do
          (:erlang.monotonic_time(:microsecond) - start_time) / 1000  # Convert to ms
        else
          nil
        end
  ...
  def test_concurrent_operations(_pool_info, operations, opts \\ []) do
    ...
    instrumented_operations = if track_performance do
    ...
    {operation_results, durations} = if track_performance do
      Enum.unzip(results)
    else
      {results, []}
    end
    ...
    parallelism_result = if verify_parallelism and track_performance do
    ...
    case parallelism_result do
      {:ok, parallelism_stats} ->
        success_count = Enum.count(operation_results, fn 
          {:ok, _} -> true
          _ -> false
        end)
        ...
        Logger.info("Concurrent operations completed: #{success_count}/#{length(operations)} successful")
        {:ok, final_result}
  ...
  def verify_session_affinity(pool_info, opts \\ []) do
    ...
    Logger.info("Verifying session affinity for #{session_count} sessions, #{operations_per_session} ops each")
    
    # Create test sessions
    session_ids = for i <- 1..session_count do
    ...
    session_results = for session_id <- session_ids do
      worker_ids = for _op <- 1..operations_per_session do
    ...
    overall_success_rate = session_results
    ...
    Logger.info("Session affinity verification: #{successful_sessions}/#{session_count} sessions maintained affinity")
    {:ok, result}
  end
  ...
  defp collect_single_performance_sample(pool_info, timestamp) do
    try do
      ...
      %{
        ...
        active_sessions: status.active_sessions,
        available_workers: status.pool_size - status.active_sessions,
        utilization: if(status.pool_size > 0, do: status.active_sessions / status.pool_size, else: 0)
      }
    catch
      _error ->
        %{
  ...
  defp calculate_performance_stats(samples) do
    valid_samples = Enum.filter(samples, fn sample -> 
