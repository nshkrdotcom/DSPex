defmodule DSPex.Testing.TestMode do
  ...
  ## Usage
      ...
      config :dspex, :test_mode, :mock_adapter           # Layer 1 (default)
      config :dspex, :test_mode, :bridge_mock           # Layer 2
      config :dspex, :test_mode, :full_integration      # Layer 3
  ...
  2. Application configuration :dspex :test_mode
  ...
  def current_test_mode do
    case System.get_env("TEST_MODE") do
      nil ->
        Application.get_env(:dspex, :test_mode, @default_mode)
  ...
  def get_adapter_module do
    ...
    case mode do
      :mock_adapter -> DSPex.Adapters.Mock
      :bridge_mock -> DSPex.Adapters.BridgeMock
      :full_integration -> DSPex.Adapters.PythonPort
    end
  end
    ...
    case effective_test_mode() do
      :mock_adapter ->
        if Process.whereis(DSPex.Adapters.Mock) do
          GenServer.stop(DSPex.Adapters.Mock)
        end
  ...
  defmacro test_in_mode(modes, test_name, do: block) when is_list(modes) do
    quote do
      if DSPex.Testing.TestMode.effective_test_mode() in unquote(modes) do
        test unquote(test_name), do: unquote(block)
      else
        @tag :skip
        test unquote(
               "#{test_name} (skipped in #{DSPex.Testing.TestMode.effective_test_mode()} mode)"
             ),
             do: :ok
      end
    end
