defmodule DSPex.PythonBridge.ErrorRecoveryOrchestratorTest do
  ...
  setup do
    ...
    {:ok, orchestrator_pid} =
      ErrorRecoveryOrchestrator.start_link(
        name: :"test_orchestrator_#{System.unique_integer([:positive])}",
        # Low capacity for easier testing
        max_concurrent: 2
      )

    # Start a circuit breaker for testing
    {:ok, cb_pid} =
      CircuitBreaker.start_link(name: :"test_cb_#{System.unique_integer([:positive])}")
