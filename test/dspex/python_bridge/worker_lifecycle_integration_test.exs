defmodule DSPex.PythonBridge.WorkerLifecycleIntegrationTest do
  ...
  describe "Enhanced Worker Lifecycle Integration" do
    ...
    test "session affinity works across multiple operations", %{pool_name: pool_name} do
      ...
      # First operation
      result1 = SessionPoolV2.execute_in_session(
        session_id,
        :ping,
        %{operation: 1},
        [pool_name: actual_pool_name]
      ...
      result2 = SessionPoolV2.execute_in_session(
        session_id,
        :ping,
        %{operation: 2},
        [pool_name: actual_pool_name]
    ...
    test "pool can be configured with different worker types" do
      ...
      basic_pool_opts = [
        ...
        worker_module: DSPex.PythonBridge.PoolWorkerV2,  # Basic worker
        pool_size: 1,
        overflow: 0
      ]
      ...
      tasks = for i <- 1..5 do
        Task.async(fn ->
          ...
          SessionPoolV2.execute_in_session(
            ...
            :ping,
            %{operation_id: i},
            [pool_name: actual_pool_name, timeout: 10_000]
          )
        end)
      end
    ...
    test "telemetry events are emitted correctly" do
      ...
      case result do
        :ok ->
          ...
          assert_receive {:telemetry_event, [:dspex, :pool, :worker, :transition], measurements, metadata}, 1000
