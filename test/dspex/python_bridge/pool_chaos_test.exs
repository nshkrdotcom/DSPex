defmodule DSPex.PythonBridge.PoolChaosTest do
  ...
  @moduletag timeout: 120_000  # 2 minutes for chaos tests
  ...
  describe "Worker Failure Chaos" do
    test "single worker failure and recovery", context do
      ...
      assert {:ok, failure_result} = inject_worker_failure(pool_info, :random,
    ...
    test "multiple worker failures", context do
      ...
      assert {:ok, failure_result} = inject_worker_failure(pool_info, 0.5,
        verify_recovery: true,
      ...
      if Map.has_key?(failure_result, :recovery_result) do
        recovery = failure_result.recovery_result
        assert recovery.recovery_successful
        Logger.info("Multiple worker failure test: #{failure_result.targeted_workers} workers failed, recovery in #{recovery.recovery_time_ms}ms")
      else
    ...
    test "cascading worker failures", context do
      ...
      assert {:ok, orchestration_result} = chaos_test_orchestrator(pool_info, chaos_scenarios,
      ...
      Logger.info("Cascading failure test: #{orchestration_result.total_scenarios} scenarios, final recovery in #{orchestration_result.final_recovery.recovery_time_ms}ms")
    end
  end
  ...
  describe "Resource Exhaustion Chaos" do
    test "memory pressure simulation", context do
      ...
      assert {:ok, pressure_result} = create_memory_pressure(pool_info,
        pressure_mb: 50,  # Moderate pressure for tests
        duration_ms: 8_000,
        verify_recovery: true
      )
      ...
      if Map.has_key?(pressure_result, :recovery_result) do
        ...
        assert recovery.recovery_successful
        Logger.info("Memory pressure test: #{pressure_result.pressure_mb}MB pressure, recovery in #{recovery.recovery_time_ms}ms")
      else
    ...
    test "port corruption simulation", context do
      ...
      assert {:ok, corruption_result} = simulate_port_corruption(pool_info,
      ...
      if Map.has_key?(corruption_result, :recovery_result) do
        ...
        assert recovery.recovery_successful
        Logger.info("Port corruption test: #{corruption_result.duration_ms}ms corruption, recovery in #{recovery.recovery_time_ms}ms")
      else
  ...
  describe "Chaos Under Load" do
    test "worker failures during concurrent operations", context do
      ...
      chaos_task = Task.async(fn ->
        inject_worker_failure(pool_info, :random,
          verify_recovery: false  # Don't verify recovery during load
        )
      end)
      ...
      assert {:ok, final_recovery} = verify_pool_recovery(pool_info, chaos_data.pre_failure_state, 30_000)
      assert final_recovery.recovery_successful
      
      Logger.info("Chaos under load test: #{load_data.successful_operations}/#{load_data.total_operations} operations successful despite chaos")
    end
    ...
    test "multiple chaos scenarios during sustained load", context do
      ...
      sustained_load_task = Task.async(fn ->
      ...
      assert length(load_result) >= 3  # Should have collected some samples during sustained load
      ...
      Logger.info("Sustained chaos test: #{length(load_result)} load samples, #{chaos_data.total_scenarios} chaos scenarios")
    end
  end
  ...
  describe "Recovery Verification" do
    test "comprehensive recovery validation", context do
      ...
      chaos_scenarios = [
        ...
        {:port_corruption, [corruption_type: :random_data, duration_ms: 4000, verify_recovery: false]}
      ]
      ...
      post_chaos_operations = for i <- 1..5 do
        fn ->
          DSPex.PythonBridge.SessionPoolV2.execute_anonymous(
            :ping,
      ...
      assert {:ok, verification_result} = test_concurrent_operations(pool_info, post_chaos_operations,
        timeout: 30_000
      )
      ...
      Logger.info("Comprehensive recovery test: #{orchestration_result.total_scenarios} chaos scenarios, #{verification_result.successful_operations}/#{verification_result.total_operations} post-recovery operations successful")
    end
  end
