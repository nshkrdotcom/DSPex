defmodule DSPex.PythonBridge.PoolPerformanceTest do
  ...
  describe "Pool Performance Benchmarks" do
    test "single operation latency benchmark", context do
      ...
      benchmark = %PerformanceBenchmark{
        ...
        throughput_threshold_ops_sec: 5.0
      }
      ...
      Logger.info(
        "Single operation latency benchmark completed: P95=#{results.measurement_results.performance_summary.latency_p95_ms}ms"
      )
    end
    ...
    test "concurrent throughput benchmark", context do
      ...
      benchmark = %PerformanceBenchmark{
        ...
        # More lenient for concurrent load
      ...
      Logger.info(
        "Concurrent throughput benchmark completed: #{results.measurement_results.performance_summary.throughput_ops_sec} ops/sec"
      )
    end
    ...
    test "session affinity performance impact", context do
      ...
      Logger.info(
        "Session affinity performance: #{affinity_result.affinity_success_rate * 100}% sessions maintained affinity"
      )
    end
  end
  ...
  describe "Performance Metrics Collection" do
    test "collect comprehensive performance metrics", context do
      ...
      assert {:ok, metrics} =
               collect_performance_metrics(pool_info,
                 duration_ms: 10_000,
    ...
    test "monitor pool performance during operations", context do
      ...
      Logger.info(
        "Pool performance monitoring completed: #{monitoring_result.sample_count} samples"
      )
    end
  end
  ...
  describe "Load Testing" do
    test "concurrent operations with performance tracking", context do
      ...
      # Allow some failures
      assert result.successful_operations >= 18
      # Verify concurrent execution
      assert result.parallelism_stats.ratio < 2.0

      Logger.info(
        "Load test completed: #{result.successful_operations}/#{result.total_operations} successful, parallelism ratio: #{result.parallelism_stats.ratio}"
      )
    end
  end
    ...
    test "detect performance regressions against baseline", context do
      ...
      current_performance = %{
        performance_summary: %{
          ...
          latency_p95_ms: 1100,
          # Slight decrease
          throughput_ops_sec: 19.0
        }
      }

      # Test regression detection
      assert {:ok, regression_analysis} =
      ...
      assert {:ok, regression_analysis_bad} =
               performance_regression_detector(
                 bad_performance,
                 [baseline_performance]
               )
      ...
      Logger.info(
        "Regression detection test completed: baseline regression=#{regression_analysis.regression_detected}, significant regression=#{regression_analysis_bad.regression_detected}"
      )
    end
  end
end
