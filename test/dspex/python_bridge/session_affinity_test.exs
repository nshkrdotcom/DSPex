defmodule DSPex.PythonBridge.SessionAffinityTest do
  ...
  setup do
    ...
    {:ok, pid} =
      SessionAffinity.start_link(name: test_name, cleanup_interval: 500, session_timeout: 200)
  ...
  describe "bind_session/2 and get_worker/1" do
    ...
    test "returns error for non-existent sessions", %{affinity: affinity_name} do
      assert {:error, :no_affinity} =
               SessionAffinity.get_worker("non_existent_session", affinity_name)
    end
      ...
      tasks =
        for i <- 1..50 do
          Task.async(fn ->
            ...
            # 5 different workers
            worker_id = "#{worker_base}_#{rem(i, 5)}"
