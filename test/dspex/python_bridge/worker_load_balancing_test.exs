defmodule DSPex.PythonBridge.WorkerLoadBalancingTest do
  ...
  setup do
    ...
    {:ok, pool_pid} =
      SessionPoolV2.start_link(
        name: pool_name,
        # More workers for load balancing tests
        pool_size: 4,
        overflow: 2
      )
  ...
  describe "load distribution" do
    test "requests are distributed across multiple workers", %{store_name: store_name} do
      ...
      # At least 10ms threshold
      threshold = max(avg_response_time * 3, 10.0)
      ...
      Logger.info(
        "Load balancing test - Avg: #{avg_response_time}ms, Max: #{max_response_time}ms"
      )
    end
    ...
    test "high concurrency requests are handled efficiently", %{store_name: store_name} do
      ...
      # At least 1ms threshold
      threshold = max(avg_response_time * 2, 1.0)
      ...
      Logger.info(
        "High concurrency test - #{concurrency_level} requests, #{fast_percentage}% fast"
      )
    end
  end
