defmodule DSPex.Testing.TestModeTest do
  ...
  describe "test mode detection" do
    test "defaults to mock_adapter when no configuration" do
      ...
      old_config = Application.get_env(:dspex, :test_mode)
      Application.delete_env(:dspex, :test_mode)

      assert TestMode.current_test_mode() == :mock_adapter

      # Restore config
      if old_config do
        Application.put_env(:dspex, :test_mode, old_config)
      end
    end
    ...
    test "uses application configuration when set" do
      ...
      System.delete_env("TEST_MODE")
      Application.put_env(:dspex, :test_mode, :bridge_mock)
    ...
    test "environment variable overrides application config" do
      ...
      Application.put_env(:dspex, :test_mode, :mock_adapter)
    ...
    test "handles invalid environment variable gracefully" do
      ...
      assert TestMode.get_adapter_module() == DSPex.Adapters.Mock
  ...
  describe "adapter module selection" do
    test "returns correct adapter for each mode" do
      ...
      assert TestMode.get_adapter_module() == DSPex.Adapters.BridgeMock

      TestMode.set_test_mode(:full_integration)
      assert TestMode.get_adapter_module() == DSPex.Adapters.PythonPort
