defmodule DSPex.PythonBridge.RetryLogicTest do
  ...
  setup do
    ...
    {:ok, cb_pid} =
      CircuitBreaker.start_link(name: :"test_retry_cb_#{System.unique_integer([:positive])}")
      ...
      result =
        RetryLogic.with_retry(
          fn ->
            ...
            if count == 0 do
              # Use structured error to avoid system error classification
              {:error, {:simple_error, "first attempt fails"}}
            else
  ...
  describe "backoff strategies" do
    ...
    test "jitter adds randomness to delays" do
      ...
      jitter_variance =
        Enum.reduce(delays_with_jitter, 0, fn delay, acc ->
          abs(delay - 300) + acc
        end)

      no_jitter_variance =
        Enum.reduce(delays_without_jitter, 0, fn delay, acc ->
          abs(delay - 300) + acc
        end)
    ...
    test "respects max_attempts setting" do
      ...
      RetryLogic.with_retry(
        fn ->
          ...
          # Use retryable error type
          {:error, {:retryable_error, "always fails"}}
        end,
        max_attempts: 3,
        base_delay: 10
      )
      ...
      # Should attempt at least once, possibly more based on strategy
