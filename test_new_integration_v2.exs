...
# Using the new tool calling system
...
IO.puts("🚀 Testing New Snakepit v0.4.1 Integration v2")
IO.puts("=" <> String.duplicate("=", 50))

# Test 1: Basic tool calling - use call_tool instead of execute_in_session
IO.puts("\n1. Testing tool calling...")
case Snakepit.call_tool("test_session", "ping", %{}) do
  {:ok, result} ->
  ...
  {:error, error} ->
    ...
    IO.puts("Trying alternative API...")
    case Snakepit.execute("test_session", "ping", %{}) do
      {:ok, result} ->
        IO.puts("✓ Alternative API successful: #{inspect(result)}")
      
      {:error, alt_error} ->
        IO.puts("✗ Alternative API failed: #{inspect(alt_error)}")
        System.halt(1)
    end
end
...
case Snakepit.call_tool("test_session", "check_dspy", %{}) do
...
case Snakepit.call_tool("test_session", "get_stats", %{}) do
